...
- **Post-research:** After any agent produces research output or external references (e.g. Porter docs claims, timeout behavior claims)
...
3. **Verify each claim** — use available tools (grep, glob, web search, gh CLI, curl) to check
...
**Project:** letterstory-toolbuilder
Generates client-branded micro-tools for iframe embedding, using Firecrawl for brand ingestion and Anthropic Claude for generation, deployed on Porter.
